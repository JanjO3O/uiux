...
$(document).ready(function(){
    ...
    $('.visual .ctrl_wrap .stop').on('click', function(){
        $('.visual .popup_wrap .popup').slick('slickPause');  /* 팝업을 일시정지해야하니까 선택자가 위랑 동일해야 일시정지됨 */
        $(this).hide()
     ...
     * 2. 클릭한 li만 aria-selected를 true로 변경(나머지는 false)
     * 3. 선택된 li의 button에만 <em>선택됨</em> 이라고 씀
     * 4. 선택된 li의 aria-controls에 써있는 속성값을 가져다가
     *    .tab_conts의 tab_item 들 중에 id가 같은 요소에만 active 클래스를 줌
    **************************************************/

    let tab_list = $('.find .tab_list ul li') //변수로 이름을 지정함
    let tab_name 
     ...
     * message가 스크롤되는 동안 p strong span 태그가 넓이가 넓어지면서 색상 바뀜
    *******************************************************/
    ...
    $('.adopt .list_wrap .list').slick({ // 넓이를 자유롭게 설정해서 css에서 list_item 입력한 width: clamp(240px, 36vw, 342px); 값이 적용됨
        ...
        //pauseOnHover: true, //마우스호버시 일시정지
        infinite: true, //무한반복
        variableWidth: true, //넓이를 자유롭게 설정  --> 반응형이 적용 안됨 --> css에서 쓴 넓이가 적용됨 
         ...
         responsive: [
            {
                ...
                settings: {
                    centerMode: false,  //  모바일 사이즈에서는 --> 가운데 정렬하는거 취소함.
                }
            },
        ]
    });
    ...
    //review slick
    $('.review .list_wrap .list').slick({  // --> js에서 팝업 수를 제한해서 css에서 입력한 width값 무시됨
        autoplay: false, //팝업 자동 실행
        autoplaySpeed: 3000, //팝업이 머무는 시간
        speed: 500, //팝업 전환 속도
        dots: false, //하단 페이지 버튼 (true, false)
    ...
    function footer_count(){  //함수의 선언
        footer_h = $('.footer').outerHeight()
        console.log(footer_h)
        $('.container').css('margin-bottom', footer_h)
    }

    footer_count()  //함수의 실행  -- 브라우저가 로딩되고 단 한 번 실행
    $(window).resize(function(){
        footer_count()  //함수의 실행  -- 브라우저가 리사이즈될 때마다 실행
    })
